...
void DataFlash::end()
{
    ...
    /* Don't call mySPI.end() here to allow use of SPI interface with
    another chip. */
}
...
uint8_t DataFlash::status()
{
    ...
    mySPI.transfer(DATAFLASH_STATUS_REGISTER_READ);
    /* Get result with a dummy write */
    status = mySPI.transfer(0);
 ...
 *       successive calls to mySPI.transfer() return
...
void DataFlash::readID(struct DataFlash::ID &id)
{
    ...
    /* Send status read command */
    mySPI.transfer(DATAFLASH_READ_MANUFACTURER_AND_DEVICE_ID);

    /* Manufacturer ID */
    id.manufacturer = mySPI.transfer(0);
    /* Device ID (part 1) */
    id.device[0] = mySPI.transfer(0);
    /* Device ID (part 2) */
    id.device[1] = mySPI.transfer(0);
    /* Extended Device Information String Length */
    id.extendedInfoLength = mySPI.transfer(0);
...
void DataFlash::pageRead(uint16_t page, uint16_t offset)
{
    ...
    /* Send opcode */
    mySPI.transfer(DATAFLASH_PAGE_READ);
    
    /* Address (page | offset)  */
    mySPI.transfer(pageToHiU8(page));
    mySPI.transfer(pageToLoU8(page) | (uint8_t)(offset >> 8));
    mySPI.transfer((uint8_t)(offset & 0xff));
...
byte DataFlash::Stransfer(byte data)
{
...
void DataFlash::arrayRead(uint16_t page, uint16_t offset)
{
    ...
    /* Send opcode */
#ifdef AT45_USE_SPI_SPEED_CONTROL
    mySPI.transfer(m_speed == SPEED_LOW ? DATAFLASH_CONTINUOUS_READ_LOW_FREQ :
                        DATAFLASH_CONTINUOUS_READ_HIGH_FREQ);
#else
    mySPI.transfer(DATAFLASH_CONTINUOUS_READ_LOW_FREQ);
#endif
...
void DataFlash::bufferRead(uint8_t bufferNum, uint16_t offset)
{
    ...
    if (bufferNum)
    {
        mySPI.transfer((m_speed == SPEED_LOW) ? DATAFLASH_BUFFER_2_READ_LOW_FREQ :
                                              DATAFLASH_BUFFER_2_READ);
    }
    else
    {
        mySPI.transfer((m_speed == SPEED_LOW) ? DATAFLASH_BUFFER_1_READ_LOW_FREQ :
                                              DATAFLASH_BUFFER_1_READ);

    }
#else
    mySPI.transfer(bufferNum ? DATAFLASH_BUFFER_2_READ_LOW_FREQ :
    ...
    mySPI.transfer((uint8_t)(offset >> 8));
...
void DataFlash::bufferWrite(uint8_t bufferNum, uint16_t offset)
{
    ...
    mySPI.transfer(bufferNum ? DATAFLASH_BUFFER_2_WRITE :
...
void DataFlash::bufferToPage(uint8_t bufferNum, uint16_t page)
{
    ...
    {
        mySPI.transfer(bufferNum ? DATAFLASH_BUFFER_2_TO_PAGE_WITH_ERASE :
                                 DATAFLASH_BUFFER_1_TO_PAGE_WITH_ERASE);
    }
    else
    {
        mySPI.transfer(bufferNum ? DATAFLASH_BUFFER_2_TO_PAGE_WITHOUT_ERASE :
                                 DATAFLASH_BUFFER_1_TO_PAGE_WITHOUT_ERASE);
    }
    
    /* see pageToBuffer */
    mySPI.transfer(pageToHiU8(page));
    mySPI.transfer(pageToLoU8(page));
    mySPI.transfer(0x00);
...
void DataFlash::pageToBuffer(uint16_t page, uint8_t bufferNum)
{
    ...
    /* Send opcode */
    mySPI.transfer(bufferNum ? DATAFLASH_TRANSFER_PAGE_TO_BUFFER_2 :
...
void DataFlash::pageErase(uint16_t page)
{
    ...
    /* Send opcode */
    mySPI.transfer(DATAFLASH_PAGE_ERASE);
...
void DataFlash::blockErase(uint16_t block)
{
    ...
    /* Send opcode */
    mySPI.transfer(DATAFLASH_BLOCK_ERASE);
    ...
    block >>= rightShift;
    mySPI.transfer(highByte(block)); 
    mySPI.transfer(lowByte(block));
    mySPI.transfer(0x00);
...
void DataFlash::sectorErase(int8_t sector)
{
    ...
    /* Send opcode */
    mySPI.transfer(DATAFLASH_SECTOR_ERASE);
	
    if((sector == AT45_SECTOR_0A) || (sector == AT45_SECTOR_0B))
    {
        mySPI.transfer(0x00);
        mySPI.transfer((static_cast<uint8_t>(-sector) & 0x01) << (m_bufferSize - 5));
    }
    else
    {
        uint8_t shift = m_bufferSize + m_pageSize - m_sectorSize - 16;        
        mySPI.transfer(sector << shift);
        mySPI.transfer(0x00);
    }
...
void DataFlash::chipErase()
{
    ...
    mySPI.transfer(DATAFLASH_CHIP_ERASE_0);
    mySPI.transfer(DATAFLASH_CHIP_ERASE_1);
    mySPI.transfer(DATAFLASH_CHIP_ERASE_2);
    mySPI.transfer(DATAFLASH_CHIP_ERASE_3);
        ...
        uint16_t page, uint16_t offset, uint8_t bufferNum)
{
    ...
    /* Send opcode */
    mySPI.transfer(bufferNum ? DATAFLASH_PAGE_THROUGH_BUFFER_2 :
...
int8_t DataFlash::isPageEqualBuffer(uint16_t page, uint8_t bufferNum)
{
    ...
    /* Send opcode */
    mySPI.transfer(bufferNum ? DATAFLASH_COMPARE_PAGE_TO_BUFFER_2 :
...
void DataFlash::deepPowerDown()
{
    ...
    /* Send opcode */
    mySPI.transfer(DATAFLASH_DEEP_POWER_DOWN);
...
void DataFlash::resumeFromDeepPowerDown()
{
    ...
    /* Send opcode */
    mySPI.transfer(DATAFLASH_RESUME_FROM_DEEP_POWER_DOWN);
...
void DataFlash::enableSectorProtection()
{
    ...
    mySPI.transfer(DATAFLASH_ENABLE_SECTOR_PROTECTION_0);
    mySPI.transfer(DATAFLASH_ENABLE_SECTOR_PROTECTION_1);
    mySPI.transfer(DATAFLASH_ENABLE_SECTOR_PROTECTION_2);
    mySPI.transfer(DATAFLASH_ENABLE_SECTOR_PROTECTION_3);
...
void DataFlash::disableSectorProtection()
{
    ...
    mySPI.transfer(DATAFLASH_DISABLE_SECTOR_PROTECTION_0);
    mySPI.transfer(DATAFLASH_DISABLE_SECTOR_PROTECTION_1);
    mySPI.transfer(DATAFLASH_DISABLE_SECTOR_PROTECTION_2);
    mySPI.transfer(DATAFLASH_DISABLE_SECTOR_PROTECTION_3);
...
void DataFlash::eraseSectorProtectionRegister()
{
    ...
    mySPI.transfer(DATAFLASH_ERASE_SECTOR_PROTECTION_REGISTER_0);
    mySPI.transfer(DATAFLASH_ERASE_SECTOR_PROTECTION_REGISTER_1);
    mySPI.transfer(DATAFLASH_ERASE_SECTOR_PROTECTION_REGISTER_2);
    mySPI.transfer(DATAFLASH_ERASE_SECTOR_PROTECTION_REGISTER_3);
...
uint8_t DataFlash::programSectorProtectionRegister(const DataFlash::SectorProtectionStatus& status)
{
    ...
    mySPI.transfer(DATAFLASH_PROGRAM_SECTOR_PROTECTION_REGISTER_0);
    mySPI.transfer(DATAFLASH_PROGRAM_SECTOR_PROTECTION_REGISTER_1);
    mySPI.transfer(DATAFLASH_PROGRAM_SECTOR_PROTECTION_REGISTER_2);
    mySPI.transfer(DATAFLASH_PROGRAM_SECTOR_PROTECTION_REGISTER_3);

    for(uint8_t i=0; i<sectorCount; i++)
    {
        mySPI.transfer(status.data[i]);
    }
...
uint8_t DataFlash::readSectorProtectionRegister(DataFlash::SectorProtectionStatus& status)
{
    ...
    mySPI.transfer(DATAFLASH_READ_SECTOR_PROTECTION_REGISTER);
    mySPI.transfer(0xff);
    mySPI.transfer(0xff);
    mySPI.transfer(0xff);
    ...
    for(uint8_t i=0; i<sectorCount; i++)
    {
        status.data[i] = mySPI.transfer(0);
    }
